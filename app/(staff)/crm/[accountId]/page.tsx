...
export default async function StaffAccountDetailPage({ params }: { params: { accountId: string } }) {
  ...
  const [accountContacts, recentOrders] = await Promise.all([
    ...
    db.select({ id: orders.id, total: orders.total, status: orders.status, createdAt: orders.createdAt })
      .from(orders).where(eq(orders.customerId, params.accountId)).orderBy(desc(orders.createdAt)).limit(5),
  ])
    ...
    <div className="p-8 space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <Card>
          ...
          <CardContent>
            {accountContacts.length === 0 ? <p className="text-sm text-muted-foreground">No contacts.</p>
        ...
        <Card>
          ...
          <CardContent>
            {recentOrders.length === 0 ? <p className="text-sm text-muted-foreground">No orders.</p>
              : recentOrders.map(o => (
