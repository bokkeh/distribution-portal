...
export default async function StaffContactsPage({ params }: { params: Promise<{ accountId: string }> }) {
  ...
  return (
    <div className="p-4 sm:p-8 space-y-6">
      <div className="flex items-center gap-4">
        <Link href={`/staff/crm/${accountId}`}><Button variant="ghost" size="icon"><ArrowLeft className="w-4 h-4" /></Button></Link>
        <div>
            ...
            {accountContacts.length === 0 ? (
              ...
              <div key={contact.id} className="flex items-start gap-3 p-3 border rounded-lg">
                ...
                <div className="flex-1">
                  ...
                  {contact.phone ? <PhoneSmsButton phone={contact.phone} recipientName={contact.name} className="text-xs" /> : null}
                </div>
              </div>
            ))}
          </CardContent>
