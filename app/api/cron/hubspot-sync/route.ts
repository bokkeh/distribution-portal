...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    console.error('[cron] hubspot-sync failed:', error)
    await postGoogleChatCard('⚠️ Cron Failure — hubspot-sync', message).catch(() => {})
    return NextResponse.json({ error: 'hubspot-sync failed', detail: message }, { status: 500 })
  }
}
